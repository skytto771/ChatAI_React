...
const ParticlesBackground = () => {
        ...
        const p = {
            ...
            update(time: number) {
                ...
                this.currentOpacity = Math.max(
                    0.08,
                    Math.min(0.75, this.opacity + Math.sin(time * this.pulseSpeed + this.pulseOffset) * 0.2)
                );
            },
    ...
    useEffect(() => {
        ...
        const drawConnections = (ctx: CanvasRenderingContext2D, particles: Particle[], mouse: { x: number; y: number }) => {
            for (let i = 0; i < particles.length; i++) {
                for (let j = i + 1; j < particles.length; j++) {
                    const dx = particles[i].x - particles[j].x;
                    const dy = particles[i].y - particles[j].y;
                    const dist = Math.sqrt(dx * dx + dy * dy);
                    const maxDist = (particles[i].connectionDist + particles[j].connectionDist) / 2;
                    if (dist < maxDist) {
